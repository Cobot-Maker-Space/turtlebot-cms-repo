...
# Ensure workspace build/install/log exist and are writable by the build user
# Create directories if missing, attempt to fix ownership (using sudo if available),
# then remove only contents (not the directories themselves).
$SUDO mkdir -p "$WORKSPACE_DIR/build" "$WORKSPACE_DIR/install" "$WORKSPACE_DIR/log" || true
if [ -n "$SUDO" ]; then
  echo "Fixing ownership of workspace directories using sudo..."
  $SUDO chown -R $(id -u):$(id -g) "$WORKSPACE_DIR" || true
fi

echo "Cleaning build/install/log directories (only inside) ..."
if [ -n "$SUDO" ]; then
  $SUDO rm -rf "${WORKSPACE_DIR}/build/"* "${WORKSPACE_DIR}/install/"* "${WORKSPACE_DIR}/log/"* || true
else
